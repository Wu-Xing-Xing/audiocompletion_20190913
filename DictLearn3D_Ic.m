function D = DictLearn3D(I0,Is,sparsity,Ic)
...
D{1} = normalize(randn(Is(1),round(Ic(1))),'norm');
D{2} = normalize(randn(Is(2),round(Ic(2))),'norm');
D{3} = normalize(randn(Is(3),round(Ic(3))),'norm');
